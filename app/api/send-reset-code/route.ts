...
import { buildResetEmail } from "@/emails/reset-password"
...
export async function POST(request: Request) {
    try {
        ...
        const ipLimit = await rateLimit({ key: `send-reset:ip:${ip}`, limit: 10, windowSeconds: 3600 })
        if (!ipLimit.ok) return rateLimitResponse(ipLimit.retryAfter)

        const emailLimit = await rateLimit({ key: `send-reset:email:${email}`, limit: 5, windowSeconds: 3600 })
        ...
        if (existing) {
            ...
            await resend.emails.send({
                ...
                to: email,
                subject: "Kod do zmiany hasła — Lendigo",
